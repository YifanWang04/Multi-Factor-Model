...
- **REBALANCE_DATE_OFFSET**：调仓日偏移（交易日数），正数=提前、负数=延后、0=不偏移；在 `strategy_config.py`、`run_detailed_backtest_report.py`、`walk_forward_config.py` 及 `run_rebalance_day.py` 中均可配置
