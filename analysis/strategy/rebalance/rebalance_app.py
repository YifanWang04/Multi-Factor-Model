...
# Moved from analysis/strategy/run_rebalance_day.py; the old path is now a CLI wrapper.
...
_STRATEGY_DIR = os.path.dirname(_HERE)
...
for _p in (_STRATEGY_DIR, _HERE, _SF_DIR, _MF_DIR, _ROOT):
...
def _price_filename() -> str:
    ...
    return price_filename(DATA_START_OFFSET_DAYS)
...
def main(
    ...
    output_path = os.path.join(reports_dir, "rebalance_day_report.xlsx")
    write_rebalance_day_report(
        result=result,
        status=status,
