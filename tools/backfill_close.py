...
def find_latest_output_price_file() -> str | None:
    ...
    pattern = os.path.join(_ROOT, "output", "rebalance_runs", "*", "data", "us_top100_daily_2023_present.xlsx")
    files = sorted(glob.glob(pattern), key=os.path.getmtime, reverse=True)
    if files:
        return files[0]
    legacy_pattern = os.path.join(_ROOT, "output", "rebalance_day_*", "data", "us_top100_daily_2023_present.xlsx")
    legacy_files = sorted(glob.glob(legacy_pattern), key=os.path.getmtime, reverse=True)
